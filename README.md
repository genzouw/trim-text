...
This repository's CI/CD uses **only services that are free for public OSS repositories**. Workflows that require paid / metered LLM API keys (e.g. `GEMINI_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) as GitHub Secrets are **not accepted** and PRs adding them will be closed.
