...
- **目的**: プルリクエスト作成時にGoogleのGemini AI APIを使用してコードの変更内容を自動でレビューし、改善提案を行います。
- **設定ファイル**: `.github/workflows/ai-codereviewer.yml`
- **特徴**: GitHub Actionsを利用してPRコードの変更分を取得し、Geminiのモデル（例: `gemini-2.5-pro`）を用いて自然言語によるコードレビューを提供します。
- **事前設定**:
  1. Google AI StudioでGemini APIキーを取得してください。
  2. GitHubリポジトリのSecretsに`GEMINI_API_KEY`という名前でAPIキーを設定してください。
