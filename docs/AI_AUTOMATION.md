...
- **目的**: JTF標準スタイルガイドに基づき、Markdownドキュメントの日本語文章を自動校正します。表記揺れや文法の誤りを検出し、ドキュメントの品質を向上させます。
- **設定ファイル**: `.textlintrc.json`, `.github/workflows/textlint.yml`
- **特徴**: PR時にreviewdogと連携し、Pull Requestのインラインコメントとして指摘を行います。完全無料で利用可能です。
- **事前設定**:
  1. 特に追加の設定は不要です。GitHub Actions上で自動的に実行されます。
