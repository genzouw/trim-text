...
### ポリシー例外ツール（リポジトリオーナー承認済み）

以下のツールは API キーを GitHub Secrets として要求しますが、無料で利用可能な代替手段が存在するため、リポジトリオーナーの承認のもとで例外として許可されています。

| ツール                  | Secret 名                           | 無料代替手段                                                                               |
| ----------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------ |
| Open Code Review        | `OCR_LLM_AUTH_TOKEN`, `OCR_LLM_URL` | 無料枠のある LLM プロバイダ（例: Groq 無料 API）またはローカル LLM（例: Ollama）を指定可能 |
| Gemini AI Code Reviewer | `GEMINI_API_KEY`                    | Google AI Studio の無料枠（パブリックリポジトリ向け）                                      |

これらはオプトイン型ツールです。Secrets を設定しない場合はワークフローが機能しないだけで、リポジトリ全体への影響はありません。
