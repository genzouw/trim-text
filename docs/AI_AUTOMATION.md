...
- **目的**: AIを活用したコード品質ゲートとして、ハルシネーションの検出、古くなったAPIの検出、ロジックの隙間の検出を行います。
- **設定ファイル**: `.github/workflows/open-code-review.yml`
- **特徴**: オープンソースであり、従来のLintでは検出できないAI固有のバグや品質低下を自動的にスコアリングして可視化します。自動的に `GITHUB_TOKEN` を利用して実行されます。
- **事前設定**:
  1. L1スキャン（構造的スキャン）を利用する場合、特別な設定は不要です。より詳細なL2/L3スキャンを利用したい場合はOllamaなどのAPIプロバイダ設定をワークフローに追加する必要があります。
