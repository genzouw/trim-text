...
- **特徴**: `actions/ai-inference` アクションを使用し、GitHub Modelsの推論API（使用モデルはワークフロー定義 `.github/workflows/pr-policy-checker.yml` の `model:` を参照。二重管理を避けるためモデル名は本ドキュメントには固定記載しません）と組み込みの `GITHUB_TOKEN` を利用して動作します。外部の有料 LLM API キーを必要とせず、完全無料でチェックを実現します。
- **事前設定**:
  1. GitHub Models が組織（Organization）またはエンタープライズレベルで有効化されている必要があります。
  2. 組織のオーナー権限で `Settings` > `Code, planning, and automation` > `Models` > `Development` へアクセスし、GitHub Models を有効化してください。
- **既知の制限**: `pull_request` トリガーで動作するため、フォークからの Pull Request では `GITHUB_TOKEN` が読み取り専用に降格し、`models: read` / `pull-requests: write` が使えません。フォーク由来の PR では推論・コメント投稿の両方が失敗します（ジョブ自体は `|| echo` により失敗扱いにはなりません）。
- **注意（プロンプトインジェクション）**: PR タイトル・本文を LLM プロンプトへ直接埋め込んで判定しているため、プロンプトインジェクションの影響を受ける可能性があります。本チェックは PR のマージを妨げない参考情報として設計されており、マージ判定のゲートとして利用しないでください。
