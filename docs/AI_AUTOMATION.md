...
- **目的**: プルリクエストがクローズ（マージまたは破棄）された際に、そのプルリクエストに関連づけられた不要な GitHub Actions キャッシュを自動的に削除し、リポジトリのキャッシュ容量（既定の無償容量10GB。リポジトリまたは組織の設定で変更可能）の圧迫やメインブランチのキャッシュの意図せぬ eviction を防ぎます。
