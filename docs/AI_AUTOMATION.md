...
- **目的**: CI (bats) で実行したユニットテストの結果を JUnit 形式で出力し、その結果を GitHub Actions の UI や Pull Request のコメントとして分かりやすく表示します。これにより、開発者が CI ログを深く追うことなく、テストの成功・失敗・スキップの詳細を即座に把握できるようになります。
