...
### 2.2 リポジトリに導入済みのツール (重複 PR を作らないこと)
...
## 3. 大原則 (MUST): CI/CD では「無料サービスのみ」を利用する
...
## 4. Constraints / 禁止事項 (MUST NOT — DO NOT submit such PRs)

以下に該当する PR は **作成しないでください**。作成しても自動的にクローズされます。これは絶対的な境界条件 (boundary) です。

### 4.1 (MUST NOT) 従量課金型 API キーを要求するワークフロー / Action の新規追加
...
### 4.2 (MUST NOT) 有料プラン / 有料ライセンスを必要とするサービスの CI 組み込み

- 無料トライアル中のサービスや、シート課金が発生するサービスも禁止です。

### 4.3 (MUST NOT) 公開 OSS でも有料となる SaaS の追加

- 例: 公開リポジトリでも Pro プラン以上を要求するもの。

### 4.4 (MUST NOT)「無料枠内に収まる前提」での従量課金 API 利用

- レート制限・利用量制限により破綻するため禁止です。

## 5. Permissions / 許可されているもの (MAY)
...
### 6.2 Bad Example (MUST NOT — 自動クローズ)
