...
#
# 各パターンは .github/PULL_REQUEST_TEMPLATE.md の文言に対する部分一致である。
# 語尾 (「完全無料であり」/「完全無料である」) のような揺れを取り込むと、
# テンプレートどおりに記入した PR が通らなくなるため、パターンは語尾を含めず
# 項目を一意に識別できる最小限の範囲に留める。テンプレートとの乖離は
# tests/check-pr-policy.bats の「PULL_REQUEST_TEMPLATE.md をそのまま記入した
# 本文が通過する」テストで検出する。
